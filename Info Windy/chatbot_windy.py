...
# Mode debug : le serveur peut démarrer sans clé API (fonctionnalités chatbot désactivées)
CEREBRAS_API_KEY = os.environ.get("CEREBRAS_API_KEY")
CEREBRAS_AVAILABLE_FOR_CHATBOT = False

if not CEREBRAS_API_KEY:
    logger.warning("⚠️  Clé API Cerebras manquante - Mode debug activé")
    logger.info("💡 Le serveur peut démarrer, mais les fonctionnalités chatbot seront désactivées")
    logger.info("📝 Pour activer le chatbot, configurez votre clé API avec: python setup_env.py")
    logger.info("📖 Ou créez un fichier .env avec: CEREBRAS_API_KEY=votre_cle_ici")
else:
...
# Disponibilité globale de Cerebras (nécessite à la fois la clé API et le module)
...
RÉPONSE CLAIRE ET SYNTHÉTIQUE BASÉE SUR LE CONTEXTE (sans troncature) :""".format(context=context, question=question)
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        if not CEREBRAS_AVAILABLE:
            return {
                "available": False,
                "message": "Chatbot non disponible - Clé API Cerebras manquante"
            }
...
def get_chatbot() -> WindyChatbot:
    ...
    if _chatbot_instance is None:
        if not CEREBRAS_AVAILABLE:
            logger.warning("⚠️  Chatbot initialisé en mode dégradé (clé API manquante)")
