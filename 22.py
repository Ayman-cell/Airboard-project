...
# Utilisez setup_env.py pour configurer vos clés ou créez un fichier .env

def get_required_env(key, description):
    """Récupère une variable d'environnement requise ou lève une exception"""
    value = os.getenv(key)
    if not value:
        error_msg = (
            f"❌ Clé API manquante: {key}\n"
            f"💡 {description}\n"
            f"📝 Pour configurer vos clés API, exécutez: python setup_env.py\n"
            f"📖 Ou créez un fichier .env avec: {key}=votre_cle_ici"
        )
        # Essayer d'afficher via Streamlit si disponible
        try:
            st.error(f"❌ Clé API manquante: {key}")
            st.info(f"💡 {description}")
            st.info("📝 Pour configurer vos clés API, exécutez: python setup_env.py")
            st.stop()
        except:
            # Si Streamlit n'est pas disponible, lever une exception
            raise ValueError(error_msg)
    return value

# Configuration Cerebras - vérification différée lors de l'utilisation
def get_cerebras_config():
    """Récupère la configuration Cerebras avec vérification des clés"""
    return {
    "gpt-oss-120b": {
            "api_key": get_required_env("CEREBRAS_GPT_OSS_120B_KEY", "Clé API Cerebras GPT-OSS-120B requise"),
        "endpoint": os.getenv("CEREBRAS_ENDPOINT", "https://api.cerebras.ai/v1/completions")
    },
    "qwen-3-235b-a22b-instruct-2507": {
            "api_key": get_required_env("CEREBRAS_QWEN_235B_KEY", "Clé API Cerebras Qwen-3-235B requise"), 
        "endpoint": os.getenv("CEREBRAS_ENDPOINT", "https://api.cerebras.ai/v1/completions")
    },
    "llama-3.3-70b": {
            "api_key": get_required_env("CEREBRAS_QWEN_32B_KEY", "Clé API Cerebras Llama-3.3-70B requise"),
        "endpoint": os.getenv("CEREBRAS_ENDPOINT", "https://api.cerebras.ai/v1/completions")
    }
}

def get_gemini_config():
    """Récupère la configuration Gemini avec vérification de la clé"""
    return {
        "api_key": get_required_env("GEMINI_API_KEY", "Clé API Google Gemini requise")
    }

# Variables globales pour les configurations (initialisées à None)
# Elles seront initialisées de manière différée lors de la première utilisation
CEREBRAS_CONFIG = None
GEMINI_CONFIG = None

# Initialisation des configurations (avec vérification des clés)
# Les clés sont vérifiées uniquement lors de l'utilisation réelle
def _init_configs():
    """Initialise les configurations si nécessaire"""
    global CEREBRAS_CONFIG, GEMINI_CONFIG
    if CEREBRAS_CONFIG is None:
        CEREBRAS_CONFIG = get_cerebras_config()
    if GEMINI_CONFIG is None:
        GEMINI_CONFIG = get_gemini_config()
    return CEREBRAS_CONFIG, GEMINI_CONFIG

# Initialisation Gemini (différée)
def _init_gemini():
    ...
    def __init__(self):
        try:
            ...
            try:
                ...
                print(f"Erreur initialisation Gemini: {str(e)}")
